...
# InOrbit Kuka AMR Fleet Connector

[![Python 3.13](https://img.shields.io/badge/python-3.13-blue.svg)](https://www.python.org/downloads/)

InOrbit Edge connector for Kuka AMR Fleet

## Overview

This repository contains the [InOrbit](https://inorbit.ai/) Edge Connector for [Kuka AMR Fleet](https://example.com).

This integration requires the Connector to be configured following the instructions below.

## API Documentation

This connector implementation is based on the official publicly available Kuka AMR Fleet API specifications available on the Kuka AMR Fleet support portal. For detailed API documentation and reference materials, please refer to:
...
- Annotation synchronization for waypoint positions between Kuka AMR Fleet and InOrbit
...
- Access to a Kuka AMR Fleet server with API credentials
- Network connectivity between the connector host and Kuka AMR Fleet server
...
- Copy `config/fleet.example.yaml` to `config/my_fleet.yaml` and configure your robot fleet. Each robot needs an InOrbit `robot_id` and the corresponding Kuka AMR Fleet `fleet_robot_id`.

- Optionally, configure the Kuka AMR Fleet connector-specific settings via environment variables. Copy `config/example.env` to `config/.env` and fill in the values. Any `connector_config` fields can be set using the `INORBIT_KUKA_AMR_FLEET_` prefix (e.g., `INORBIT_KUKA_AMR_FLEET_FLEET_HOST`, `INORBIT_KUKA_AMR_FLEET_FLEET_USERNAME`). Environment variables are used as fallbacks when fields are missing from the YAML configuration. See `config/example.env` for reference. The `.env` file will be automatically loaded when the connector is run.
...
```bash
source config/.env && kuka-amr-fleet-connector -c config/my_fleet.yaml
```
...
- **Issues**: [GitHub Issues](https://github.com/inorbit-ai/kuka-amr-fleet-connector/issues)
